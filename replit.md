...
- **X/social share cards for reports**: report links use hash fragments (`/#jcat-0001`) which crawlers never see, so every report gets its own static share page at `/r/<case-id>/` with its own OG tags + instant redirect. To add a card for a NEW report: add an entry to `artifacts/space-report/scripts/share-cards.json`, run `node artifacts/space-report/scripts/generate-share-pages.mjs`, republish, then post links as `www.planet42069.org/r/<case-id>/`.
